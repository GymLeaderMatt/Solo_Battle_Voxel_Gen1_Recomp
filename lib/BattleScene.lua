...
local function castShadows(state, arena, terrain, nbMesh, cx, cy, vw, vh,
  ...
  -- the platforms are the only receiver a disc rung has; without them a mon
  -- casts onto nothing and the pair reads as floating
  if arena and arena.discs then
    V.require("DiscStage").cast(ShadowMap, arena,
                                BattleScene.groundY(host, arena))
  else
...
function BattleScene.groundY(map, arena)
  -- a disc rung's platforms ARE the ground, and there is no terrain under
  -- them to sample a height from
...
function BattleScene.render(state, arena, textures, token, battle)
  ...
  -- A DISC RUNG (3D-BTL B): two carried platforms against the sky, no map in
  -- the shot. Shares every skip a flat plate makes, but it is not a flat
  -- plate -- it has real geometry that casts and receives, and what is behind
  -- it is the hour's own sky rather than a backplate.
  local discs = arena.discs and true or false
  if discs then whiteFill = false end
  local flatFill = whiteFill
  ...
  Voxel3D.camera = nil
  if flatFill and not discs then
  ...
  -- ------- a disc rung has no map in the shot, so the sky IS the picture
  --
  -- Two sources, and the stage decides which. A stage marked "cycle" is
  -- handed to Sky.dress and stands under the hour's own bands, which is what
  -- every outdoor stage wants. Anything else carries its own fixed ramp,
  -- because a cave does not care what time it is.
  --
  -- Either way the result is a flat fill with a `bands` array on it, so the
  -- band pass that already exists renders both and there is no second path.
  if discs then
    local okStage, Stages = pcall(V.require, "BattleStages")
    local entry = okStage and Stages and Stages.forBattle(state.map, battle)
    local dressed = nil
    if entry then
      local ok, out = pcall(Stages.dress, entry, sky)
      if ok then dressed = out end
      -- and the ground it names, for DiscStage to pick its texture with
      local DiscStage = V.require("DiscStage")
      DiscStage.stageName = entry.disc
    end
    if dressed then
      sky = dressed
    else
      -- A "cycle" stage takes the hour's bands, and takes them whatever the
      -- engine calls this map.
      --
      -- This used to be gated on VoxelScene.skyColor being non-nil, i.e. on
      -- the map being OUTDOOR. That gate belongs to the A rung, where indoors
      -- means there is a ceiling in the shot. A disc rung draws no map at all,
      -- so the classification has nothing to be about -- and Viridian Forest,
      -- which Gen 1 classes as an interior, fell through both branches and
      -- kept the flat INDOOR_SHADE fill. All blue, no gradient.
      --
      -- Only stages routed to "cycle" reach here, and every one of them
      -- (grass, forest, rocky, sandy, town, surf) is somewhere that sees sky.
      local okDress, hour = pcall(V.require("Sky").dress, sky)
      if okDress and hour then
        sky = hour
        -- and grade the whole frame, not Sky.SPAN of it: same reason the
        -- fixed ramps set this. There is no world below the horizon here.
        sky.fullBands = true
      end
    end
  end
  ...
  local ok, err = pcall(function()
    ...
    if discs then
      V.require("DiscStage").draw(arena, groundY)
    elseif not flatFill then
