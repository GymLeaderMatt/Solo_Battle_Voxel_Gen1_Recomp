...
DiscStage.RADIUS = 18 * 1.3
DiscStage.MAX_RADIUS = 34 * 1.3
DiscStage.PAD = 1.8 * 1.3
...
function DiscStage.radiusFor(r)
  local want = (r or 0) * DiscStage.PAD
  if want < DiscStage.RADIUS then return DiscStage.RADIUS end
  if want > DiscStage.MAX_RADIUS then return DiscStage.MAX_RADIUS end
  return want
end
...
-- ------- the painted discs
--
-- A stage names a PNG in assets/battle/discs. The file is the only wiring:
-- data/battle_stages.lua picks the name, this loads it, and a name with no
-- file behind it falls back to the generated platform below rather than
-- leaving a Pokemon standing on nothing.
--
-- Authored TOP-DOWN on a square canvas. The quad lies on the ground plane
-- and the camera does the foreshortening, so a pre-squashed ellipse would be
-- squashed twice.
local painted = {}
local texture = nil

function DiscStage.paintedTexture(name)
  if not name then return nil end
  if painted[name] ~= nil then return painted[name] or nil end
  local ok, img = pcall(function()
    -- through the mod's asset resolver, never a bare relative path: the mod
    -- is mounted, so "assets/..." on its own resolves against the GAME's
    -- root and silently finds nothing
    local path = V.mod.assets:path("assets/battle/discs/" .. name .. ".png")
    local image = love.graphics.newImage(path)
    -- nearest, like every other texture in this mode: these are pixel art and
    -- are meant to read as texels. Clamped so the rim does not wrap.
    image:setFilter("nearest", "nearest")
    image:setWrap("clampzero", "clampzero")
    return image
  end)
  painted[name] = (ok and img) or false
  return painted[name] or nil
end

-- What the current fight stands on. Set by BattleScene when the stage
-- resolves; nil until a battle is staged, and nil for missing art.
DiscStage.stageName = nil

function DiscStage.activeTexture()
  return DiscStage.paintedTexture(DiscStage.stageName) or DiscStage.texture()
end
...
local function grain(x, y)
  local bx = (x - x % DiscStage.GRAIN) / DiscStage.GRAIN
  local by = (y - y % DiscStage.GRAIN) / DiscStage.GRAIN
...
function DiscStage.mesh()
  if mesh == nil then mesh = build() or false end
  return mesh or nil
end

function DiscStage.invalidate()
  for _, img in pairs(painted) do
...
-- How much of the tuned radius a PAINTED disc actually covers.
--
-- RADIUS itself is not touched: it is solved so the two platforms come within
-- 1.2 world pixels of touching at three cells apart and never intersect, and
-- the shadow pass is registered against it. But the generated platform faded
-- to nothing at its rim, where a painted one has a hard edge and often a
-- bright band -- at full width that stops being ground and becomes the
-- loudest thing in the frame. Insetting here keeps every texel of the source
-- and leaves the tuning alone.
--
-- 0.42 rather than the 0.85 this shipped with. At full width a painted disc
-- is 2.9 map cells across and about three times the width of the Pokemon
-- standing on it -- which the GENERATED platform could carry, because it
-- faded to nothing at its rim, and a painted one cannot. Half is the number
-- to start from; it is one constant and worth moving until it looks right.
DiscStage.PAINTED_INSET = 0.42

function DiscStage.matrix(x, groundY, z, radius)
  radius = radius or DiscStage.RADIUS
  if DiscStage.paintedTexture(DiscStage.stageName) then
    radius = radius * DiscStage.PAINTED_INSET
  end
  return Mat4.mul(Mat4.translate(x, groundY - DiscStage.SINK, z),
...
local function each(arena, groundY, fn)
  ...
  for _, side in ipairs({ "enemy", "player" }) do
    ...
    if cell then
      ...
      fn(DiscStage.matrix(cell[1], groundY, cell[2],
                             DiscStage.radiusFor(footprint)))
    end
  end
end
...
-- months.
DiscStage.ORIGIN = { 16, 16 }

function DiscStage.arena(map)
  local BattleArena = V.require("BattleArena")
  local arena = BattleArena.at(DiscStage.ORIGIN[1], DiscStage.ORIGIN[2],
                               "wide")
...
function DiscStage.draw(arena, groundY)
  if not (arena and arena.discs) then return end
  local m = DiscStage.mesh()
  local tex = DiscStage.activeTexture()
  if not (m and tex) then return end
  Voxel3D.seams(false)
  Voxel3D.glass(false)
  -- ------- why the painted discs blend
  --
  -- The scene shader discards under half alpha, which is what keeps a
  -- sprite's transparent corners out of the depth buffer. The generated
  -- platform was built for that: its rim is an ordered dither, every texel
  -- fully on or fully off. A painted rim is a smooth curve, and the same rule
  -- cuts it into a notched circle.
  --
  -- A B rung has no terrain behind the discs to z-fight against, so there is
  -- nothing here for the depth write to protect. Blending is free exactly
  -- here and nowhere else.
  local blended = DiscStage.paintedTexture(DiscStage.stageName) ~= nil
  if blended then Voxel3D.alphaBlend(true) end
  each(arena, groundY, function(matrix) Voxel3D.draw(m, tex, matrix) end)
  if blended then Voxel3D.alphaBlend(false) end
  Voxel3D.glass(true)
  Voxel3D.seams(true)
end
...
function DiscStage.cast(shadowMap, arena, groundY)
