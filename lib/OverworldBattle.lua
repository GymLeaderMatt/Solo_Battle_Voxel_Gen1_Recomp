...
-- The GB rows the two blocks hang from, in the 160x144 frame, so they hold
-- their place on any window. The player's block hangs DOWN from its row and
-- the foe's BAND hangs UP from its one -- the band rather than the block on
...
-- the HUD keeps the same relative size on any display rather than being a
-- fixed pixel count that looks large on a laptop and small on a 4K panel.
OverworldBattle.HUD_TARGET_W = 280
OverworldBattle.HUD_TARGET_REF_W = 1920

-- Whether the scale is held to whole rungs. OFF hits the target exactly and
-- lets glyph strokes come out a pixel uneven; the block sits at a fixed place
-- and the scale never changes, so that unevenness is static rather than
-- something that crawls.
OverworldBattle.HUD_PIXEL_SNAP = false

-- The scale that keeps the wider block inside the target.
function OverworldBattle.hudRung(pw)
  ...
  if OverworldBattle.HUD_PIXEL_SNAP then rung = math.floor(rung) end
...
function OverworldBattle.snapRects(shot)
  ...
  -- the battle zoom), and is where the width target applies. OG is left alone
  -- deliberately -- shrinking it there would break the alignment it exists for.
  if OverworldBattle.hudScaleSetting:get() == "og" then
    hs = s
  else
    hs = math.min(hs, OverworldBattle.hudRung(shot.pw))
  end
  local e, p = OverworldBattle.HUD_RECT.enemy, OverworldBattle.HUD_RECT.player
  local inset = OverworldBattle.HUD_EDGE_INSET
  -- Each block is hung off its own window edge by the inset, and its whole band
  -- travels with it, so long names, HUD shakes and the intro pokeball rows keep
  -- the internal alignment the engine drew them with.
  local ex, px
  if OverworldBattle.HUD_SIDES_SWAPPED then
    ...
    ex = (inset - e[1]) * hs
    -- Unswapped, the player block keeps the engine's own status-box origin
    -- flush at the right edge rather than taking the inset: an external EXP-bar
    -- mod anchors to that box and would otherwise sit two pixels off it.
    px = shot.pw - (p[1] + p[3]) * hs
  end
  -- Where each BAND lands, since the band is what is actually blitted; each
  -- block's own top then follows from its offset inside its band. The foe's
  -- band is hung by its bottom edge and the player's by its block's top edge,
  -- per HUD_PIN above.
  local eBand = OverworldBattle.HUD_BAND.enemy
  local pBand = OverworldBattle.HUD_BAND.player
  local pin = OverworldBattle.HUD_PIN
  local eBandY = shot.ly + pin.enemyBottom * s - eBand[4] * hs
  local pBandY = shot.ly + pin.playerTop * s - (p[2] - pBand[2]) * hs
  local eTop = eBandY + (e[2] - eBand[2]) * hs
  local pTop = pBandY + (p[2] - pBand[2]) * hs
  local rects = {
