...
function BattleExit:update()
  ...
  live = nil
  if self.battle then self.battle.soloBattleLeaving = nil end
end
...
function BattleExit.install()
  ...
  if not BattleState.soloBattleExitHook then
    ...
    function BattleState:finish()
      if self.soloBattleLeaving or not BattleExit.wanted(self) then
        return inner(self)
      end
      self.soloBattleLeaving = true
      BattleExit.start(self, function() inner(self) end)
    end
    BattleState.soloBattleExitHook = true
  end

  local Renderer = require("src.render.Renderer")
  if not Renderer.soloBattleExitHook then
    ...
    Renderer.soloBattleExitHook = true
  end
end
