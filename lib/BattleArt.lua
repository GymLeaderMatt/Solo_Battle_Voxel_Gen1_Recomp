...
-- ANIMATED is gone with the atlases it decoded. STATIC is the default and
-- the only mode the oversized-master pipeline works in (see OverworldBattle's
...
BattleArt.duplicateSetting = ModSetting.new(
  ...
  { "battle_art", "modded" }, { "BATTLE ART", "MODDED" }, 1)
...
local function trainerKey(battle)
  ...
  if id == "OPP_ROCKET" and (battle.soloBattleTrainerParty or 1) >= 42 then
    return "jessie-james"
  end
