...
local SHADER = [[
  ...
  uniform float alphaBlendOn; // 0 = key the alpha, which is every pass but
                              // the painted discs (see Voxel3D.alphaBlend)
  ...
  vec4 effect(vec4 color, Image tex, vec2 tc, vec2 sc) {
    ...
    // The threshold is derived rather than sent, so that a shader which
    // never receives the uniform still keys at 0.5. GLSL defaults an
    // unset uniform to ZERO, and a zero THRESHOLD discards nothing at all
    // -- which paints every card's transparent canvas as an opaque black
    // box. There is more than one compiled variant of this shader (the
    // unlit card one among them), so "every variant was sent it" is not a
    // safe assumption to build on.
    if (p.a < (alphaBlendOn > 0.5 ? 0.02 : 0.5)) discard;
...
function Voxel3D.beginScene(w, h, cx, cy, vw, vh, sky, slot)
  ...
  -- Where the band pass stops. Normally the camera's own horizon, or
  -- Sky.SPAN (0.23) of the frame when that is out of shot -- which is the
  -- free-roam and A-rung answer, because below the horizon there is a WORLD
  -- to see and no gradient belongs over it.
  --
  -- A disc rung has no world. The sky IS the entire picture, and stopping the
  -- gradient a quarter of the way down leaves the other three quarters flat
  -- at the clear colour. So a sky marked fullBands is handed the whole frame.
  local skyHorizon = (sky and sky.fullBands) and h or Voxel3D.horizonY(h)
  Voxel3D.skyEdge = (sky and sky.bands)
                    and Sky.region(h, skyHorizon) or nil
  if sky then
    ...
    Sky.paint(w, h, sky, skyHorizon, Voxel3D.cell,
  ...
  -- keying is the default, and zero already means keying, so this is only
  -- to clear a flag a previous frame may have left set
  pcall(sh.send, sh, "alphaBlendOn", 0)
...
-- Let the next draws BLEND their edges instead of keying them.
--
-- The scene keys alpha rather than blending it, and must: discarding is what
-- keeps a sprite's transparent corners out of the depth buffer, so a model
-- never carves a hole out of whatever stands behind it. The cost is that a
-- smooth alpha ramp does not fade, it cuts -- a hard circle at wherever the
-- ramp crosses the threshold.
--
-- One pass is exempt, and only one: the painted battle discs on a B rung
-- (see DiscStage). There is no terrain behind them for a depth hole to
-- expose, so lowering the threshold and turning the depth WRITE off for the
-- length of that pass costs nothing and buys round rims. Depth TESTING stays
-- on, so a Pokemon standing on a disc still occludes it correctly.
--
-- Always paired with alphaBlend(false); leaving it set would blend the whole
-- rest of the frame and put holes in the diorama.
function Voxel3D.alphaBlend(on)
  if not (active and activeShader) then return end
  pcall(activeShader.send, activeShader, "alphaBlendOn", on and 1 or 0)
  pcall(love.graphics.setDepthMode, "lequal", not on)
end
